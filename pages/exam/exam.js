...
const questionBankOperations = {
  ...
  // 获取随机题目
  async getRandomQuestions(category, count = 1) {
    try {
      const questions = await questionApi.getQuestionsByCategory(category);
...
Page({
  ...
  handleQuestionCountChange(e) {
    // 处理题目数量输入
    const count = parseInt(e.detail.value) || 5;
    this.setData({
      questionCount: count
    });
  },
  ...
  async handleConfirmation() {
    ...
    try {
      ...
      if (examData.examType === 'specialized') {
        ...
        const categoryQuestions = await questionBankOperations.searchByCategory(category1, category2);
        ...
        questions = shuffled.slice(0, examData.questionCount);
      } else {
        // 模拟考试：按照五种规则之一抽取题目
        ...
        switch (ruleIndex) {
          case 0: // 4个穴定位+1个推拿手法
            questions = [
              ...(await questionBankOperations.getRandomQuestions('穴训练', 4)),
              ...(await questionBankOperations.getRandomQuestions('推拿', 1))
            ];
            break;
          case 1: // 4个穴定位+1种进针法+1种行针法
            const needleQuestions = await questionBankOperations.getRandomQuestions('毫针、进针与行针', 2);
            questions = [
              ...(await questionBankOperations.getRandomQuestions('穴训练', 4)),
            ...
            break;
          case 2: // 4个穴定位+1种灸法
            const moxibustionQuestions = await questionBankOperations.getRandomQuestions('毫针、进针与行针', 1);
            questions = [
              ...(await questionBankOperations.getRandomQuestions('穴训练', 4)),
              ...moxibustionQuestions.filter(q => 
                q.category2 === '艾灸法'
              )
            ];
            break;
          case 3: // 4个穴定位+1种皮肤针
            const skinNeedleQuestions = await questionBankOperations.getRandomQuestions('毫针、进针与行针', 1);
            questions = [
              ...(await questionBankOperations.getRandomQuestions('穴训练', 4)),
              ...skinNeedleQuestions.filter(q => 
                q.category2 === '皮肤针'
              )
            ];
            break;
          case 4: // 4个穴定位+1种三棱针
            const threeEdgeNeedleQuestions = await questionBankOperations.getRandomQuestions('毫针、进针与行针', 1);
            questions = [
              ...(await questionBankOperations.getRandomQuestions('穴训练', 4)),
              ...threeEdgeNeedleQuestions.filter(q => 
                q.category2 === '三棱针'
              )
            ];
            break;
          default:
            questions = await questionBankOperations.getRandomQuestions('穴训练', 5);
        }
      }
      ...
      // 跳转到考试题目页面
      wx.navigateTo({
        url: `/pages/exam-questions/exam-questions`,
